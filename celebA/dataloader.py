...
class CelebAMaskHQ(Dataset) :
    def __init__(self, train=True):
        ...
        labels_path = 'celebA/data/CelebAMask-HQ/CelebAMask-HQ-attribute-anno.txt'
        data_dict = pd.read_csv(labels_path, sep=' ', skiprows=1)
        data_dict.reset_index(inplace=True)
        data_dict.rename(columns={'level_0': 'img'}, inplace=True)
        data_dict.drop('level_1', axis=1, inplace=True)

        # Suffle the data
        data_dict = data_dict.sample(frac=1, random_state=0).reset_index(drop=True)

        if train:
            self.ims = data_dict.head(25000).img.to_list()
            self.labels = tensor(data_dict.head(25000).Male.to_numpy(), dtype=torch.long)
        else:
            self.ims = data_dict.tail(5000).img.to_list()
            self.labels = tensor(data_dict.tail(5000).Male.to_numpy(), dtype=torch.long)
    ...
    def __getitem__(self, idx):
        ims = Image.open(join('celebA/data/CelebAMask-HQ/CelebA-HQ-img', self.ims[idx])).convert('RGB')
...
def get_dataloaders(dataset, batch_size, workers):
    ...
    ds_train = CELEB(train=True)
    ds_test = CELEB(train=False)
...
if __name__ == '__main__':
    dl_train, dl_test = get_dataloaders('CelebAMask-HQ', batch_size=64, workers=4)
    print("CelebAMask-HQ")
